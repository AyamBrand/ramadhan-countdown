 ...
 */
export function usePrayerTimesFixed(zoneCode: string, year: number = 2026, month: number = 2): PrayerTimesData {
  ...
  useEffect(() => {
    const fetchPrayerTimes = async () => {
      try {
        ...
        // waktusolat.app API endpoint - SOLAT V2 (lebih baru)
        // Endpoint: GET /v2/solat/{zone}
        // Akan return data untuk bulan saat ini, tapi kita bisa parse untuk bulan tertentu
        ...
        // Response structure: { code: "SGR01", month: 2, year: 2026, hijri: "1447", prayerTimes: [...] }
        if (result && result.prayerTimes && Array.isArray(result.prayerTimes)) {
          const prayerTimes: PrayerTime[] = result.prayerTimes.map((item: any, index: number) => ({
            date: item.date || "",
            day: index + 1,
            imsak: item.imsak || "-",
            subuh: item.fajr || item.subuh || "-",
            syuruk: item.sunrise || item.syuruk || "-",
            zohor: item.dhuhr || item.zohor || "-",
            asar: item.asr || item.asar || "-",
            maghrib: item.maghrib || "-",
            isyak: item.isha || item.isyak || "-",
          }));
