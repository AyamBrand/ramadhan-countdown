 ...
 */
export function usePrayerTimesV2(zoneCode: string, year: number = 2026, month: number = 2): PrayerTimesData {
  ...
  useEffect(() => {
    const fetchPrayerTimes = async () => {
      try {
        ...
        // e-solat.gov.my API endpoint yang benar
        const url = `https://www.e-solat.gov.my/index.php?r=solat/api/getsoltable&zone=${zoneCode}&month=${dateStr}`;
        ...
        // Parse data dari API
        if (result && result.prayerTime && Array.isArray(result.prayerTime)) {
